...
- Использовать тёмный dashboard UI в стиле cybersecurity SaaS с project selector и настройкой scope.
...
7. Запустите задачи `Subdomains`, `DNS Records`, затем `HTTP Probe` из UI.
8. Убедитесь, что cards, Recon Assets и Recent Activity Log начали обновляться реальными данными.

API-проверка:

```bash
curl -H "X-API-Key: $WEB_API_KEY" http://localhost:8000/api/projects
curl -H "X-API-Key: $WEB_API_KEY" http://localhost:8000/api/projects/example.com/dashboard
curl -H "X-API-Key: $WEB_API_KEY" \
